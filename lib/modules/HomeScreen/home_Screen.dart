...
import 'package:travel/modules/HomeScreen/AddHotels.dart';
import 'package:travel/modules/HomeScreen/hotelsDetails.dart';
...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => AuthenticationCubit()..getUid()..getHotels()..getProfile(),
                      ...
                      child: IconButton(
                        onPressed: () {
                          ...
                          // GameCubit.get(context).getPassword();
                           navigateTo(context, const AddHotels());
                        },
                  ...
                  children: [
                    Padding(
                      ...
                      child: textFormField(
                          ...
                          fromLTR: false,
                          onTap: () {
                            AuthenticationCubit.get(context).getHotels();
                          },
                          onChange: (value) {
                            if (AuthenticationCubit.get(context).searchGameController.text ==
                                '' || AuthenticationCubit.get(context).hotels.isEmpty) {
                              AuthenticationCubit.get(context).getHotels();
                            }
                            AuthenticationCubit.get(context).searchAboutHotels(value);
                     ...
                     // condition: true,
                      ...
                      builder: (context) =>  ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return buildGameItem(AuthenticationCubit.get(context).hotels[index] ,context,index);
                        },
                        separatorBuilder: (context, index) =>
                            divider(10, 10, lightGrey),

                        itemCount: AuthenticationCubit.get(context).hotels.length,
                      ),
  ...
  Widget buildGameItem(HotelModel model, context,index)
  {
    if(AuthenticationCubit.get(context).id==systemId){
      return Dismissible(
        ...
        onDismissed: (_) {
          AuthenticationCubit.get(context).deleteHotels(model.id);

        },
        key: UniqueKey(),
        child: InkWell(
          onTap: () async {
            navigateTo(context,  HotelDetails(model));
          },
          child:Padding(
